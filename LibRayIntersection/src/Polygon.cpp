...
bool CPolygon::PolygonEnd()
{
    ...
    do
    {
        ...
        std::vector<CGrVector>::iterator normal = m_normals.begin();
        std::vector<CGrVector>::iterator tvert = m_tvertices.begin();
        std::vector<CGrVector>::iterator cur=m_vertices.begin();
        for( ;  cur != m_vertices.end();  )
        {
    ...
    if((GetTexture() != NULL || !m_tvertices.empty())
            && m_tvertices.size() != m_vertices.size())
    {
        return false;
    }
    ...
    vector<CGrVector>::iterator a = m_vertices.begin();
    vector<CGrVector>::iterator b = a;     b++;
    vector<CGrVector>::iterator c = b;     c++;

    CGrVector cross = Cross(*c - *b, *a - *b);
...
void CPolygon::IntersectInfo(const CGrVector &intersect,  
...

{
    ...
    if(m == X)
    {
        ...
        double lz = m_vertices[a].Z() + f * (m_vertices[b].Z() - m_vertices[a].Z());
        double rz = m_vertices[c].Z() + g * (m_vertices[d].Z() - m_vertices[c].Z());
    ...
    else if(m == Z)
    {
        ...
        double lz = m_vertices[a].Y() + f * (m_vertices[b].Y() - m_vertices[a].Y());
        double rz = m_vertices[c].Y() + g * (m_vertices[d].Y() - m_vertices[c].Y());
