...
GLuint CGrTexture::TexName()
{
    ...
    if (m_mipmap)
    {
        ...
        gluBuild2DMipmaps(GL_TEXTURE_2D, 3, m_width, m_height, GL_RGB, GL_UNSIGNED_BYTE,
            m_image[0]);
    }
    else
    {
...
//
void CGrTexture::SetSize(const int p_x, const int p_y)
{
...
void CGrTexture::Set(const int x, const int y, const int r, const int g, const int b) const
{
...
void CGrTexture::Fill(const int r, const int g, const int b) const
{
    for (int i = 0; i < m_height; i++)
    {
        BYTE* img = m_image[i];
...
bool CGrTexture::LoadMemory(const BYTE* image,
                            ...
                            const int colpitch, const int rowpitch,
...
bool CGrTexture::LoadFile(const _TCHAR* pFilename)
{
    ...
    int cnt = WideCharToMultiByte(CP_UTF8, 0, pFilename, -1, vfilename.data(),
        static_cast<int>(len) + 1, nullptr, nullptr);
    filename = vfilename.data();
#else
   filename = pFilename;
#endif
...
bool CGrTexture::ReadDIBFile(istream& file)
{
    ...
    */
    ...
    const auto pBMI = reinterpret_cast<BITMAPINFOHEADER*>(new BYTE[nBMISize]);
    ...
    int r, c;
    int usewidth1 = (m_width + (PADSIZE - 1)) / PADSIZE;
    usewidth1 *= PADSIZE;
    int usewidth3 = (m_width * 3 + (PADSIZE - 1)) / PADSIZE;
    usewidth3 *= PADSIZE;
    int usewidth4 = (m_width * 4 + (PADSIZE - 1)) / PADSIZE;
    usewidth4 *= PADSIZE;
