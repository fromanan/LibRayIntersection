...
public:
    ...
    shared_ptr<CGrPoint> Sample(double u, double v, bool smoothResult = false)
    {
        auto c = int(u * Width());
        auto r = int(v * Height());
    	...
    	(
            double(m_image[r][c * 3 + 0]) / 255.,
            double(m_image[r][c * 3 + 1]) / 255.,
            double(m_image[r][c * 3 + 2]) / 255.
		);
    }

    shared_ptr<CGrPoint> Sample(CGrPoint texCoord)
    {
